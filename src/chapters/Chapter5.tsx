      ...
      >
        ...
        {answer && (
          <>
            ...
            <ContinueButton onClick={() => setStep(1)}>À toi d’estimer des tirs</ContinueButton>
          </>
        )}
      </LabShell>
    )
  ...
  if (step === 1) {
    return (
      <LabShell
        visual={
          <div className="visual-stack">
            <FootballPitch shots={scenarioShots} showLabels />
            <div className="human-xg-total"><span>Ton total actuel</span><strong>{humanTotal.toFixed(2)} xG</strong></div>
          </div>
        }
      >
        ...
        <div className="xg-estimator-list">
          {scenarioShots.map((shot) => (
            <div key={shot.id} className={touchedIds.includes(shot.id) ? 'touched' : ''}>
              <div className="xg-estimator-meta"><strong>{shot.id}</strong><span>{shot.distance.toFixed(1)} m · {shot.angle.toFixed(0)}°</span></div>
              <input
                aria-label={`Estimation xG du tir ${shot.id}`}
                type="range"
                min="0.05"
                max="0.95"
                step="0.05"
                value={estimates[shot.id]}
                onChange={(event) => updateEstimate(shot.id, Number(event.target.value))}
              />
              <strong className="xg-estimate-value">{Math.round(estimates[shot.id] * 100)}%</strong>
            </div>
          ))}
        </div>
        {touchedIds.length < scenarioShots.length ? (
          <p className="practice-gate">Ajuste les {scenarioShots.length} tirs avant de voir le modèle.</p>
        ) : (
          <button className="primary-lab-button" onClick={() => setRevealed(true)}>Comparer avec le modèle</button>
        )}
        {revealed && (
          <>
            <div className="xg-comparison-grid">
              {scenarioShots.map((shot) => {
                const modelProbability = probabilities.get(shot.id) ?? 0
                return (
                  <div key={shot.id}>
                    <span>{shot.id}</span>
                    <small>toi</small><strong>{Math.round(estimates[shot.id] * 100)}%</strong>
                    <small>modèle</small><strong>{Math.round(modelProbability * 100)}%</strong>
                  </div>
                )
              })}
            </div>
            <div className="feedback neutral">
              <strong>Écart moyen : {Math.round(averageGap * 100)} points de probabilité.</strong>
              <span>Ce n’est pas une note sur ton intuition. Ce qui compte est de voir qu’une probabilité est un nombre qu’on doit apprendre et vérifier à partir de beaucoup d’exemples.</span>
            </div>
            <ContinueButton onClick={() => setStep(2)}>Additionner les occasions</ContinueButton>
          </>
        )}
      </LabShell>
    )
  ...
  if (step === 2) {
    return (
      ...
      >
        ...
        <p className="lead">Additionne les quatre valeurs. Ne compte pas les buts réellement marqués : on additionne les probabilités.</p>
        ...
        {sumAnswer !== null && (
          <>
            <div className={`feedback ${Math.abs(sumAnswer - Number(totalXg.toFixed(1))) < 0.01 ? 'good' : 'neutral'}`}>
              ...
              <span>Ton propre total était {humanTotal.toFixed(2)} xG. Les deux représentent exactement la même opération : additionner des estimations de probabilité tir par tir.</span>
            </div>
            <UnderTheHood>
              <p>La régression logistique calcule un score à partir de la distance et de l’angle, puis le transforme en nombre entre 0 et 1 avec une <strong>sigmoïde</strong>. Ce nombre devient ici la probabilité affichée.</p>
            </UnderTheHood>
    ...
    <LabShell
      visual={
        <div className="formula-board">
          <span>tir 1</span><strong>p₁</strong><b>+</b><span>tir 2</span><strong>p₂</strong><b>+ … =</b><strong>{totalXg.toFixed(2)} xG</strong>
        </div>
      }
    >
      ...
      <div className="reveal-card"><span>Idée à garder</span><strong>Un xG de 0,20 ne prédit pas le résultat d’un tir. Il estime une fréquence de réussite pour des situations comparables.</strong></div>
