...
export function Chapter4({ step, setStep, onComplete }: Props) {
  ...
  if (step === 0) {
    return (
      <LabShell visual={<div className="score-stage"><span>Score sur les exemples appris</span><strong>{Math.round(fixedEvaluation.trainAccuracy * 100)}%</strong><small>{learningShots.length} tirs déjà vus</small></div>}>
        ...
        {trustAnswer && <><div className="feedback neutral"><strong>Le piège est dans la question.</strong><span>On est en train de noter le modèle sur des exemples qu’il connaît déjà. C’est comme réviser avec le corrigé puis repasser exactement la même feuille.</span></div><ContinueButton onClick={() => setStep(1)}>Construire un vrai contrôle</ContinueButton></>}
      </LabShell>
    )
  }
  ...
  if (step === 1) {
    return (
      <LabShell visual={<div className="split-stage"><div className="split-card train"><span>APPRENTISSAGE</span><strong>{previewSplit.train.length} tirs</strong><small>le modèle peut les utiliser</small></div><div className="split-arrow">→</div><div className="split-card test"><span>TEST</span><strong>{previewSplit.test.length} tirs</strong><small>jamais montrés au modèle</small></div></div>}>
        ...
        <div className="ratio-picker" role="group" aria-label="Part réservée au test">{[0.1, 0.3, 0.5].map((ratio) => <button key={ratio} className={testRatio === ratio ? 'selected' : ''} onClick={() => { setTestRatio(ratio); setRuns([]) }}><strong>{Math.round(ratio * 100)}%</strong><span>pour le test</span></button>)}</div>
  ...
  if (step === 2) {
    ...
    return (
      <LabShell visual={<div className="experiment-lab"><div className="experiment-headline"><span>Test réservé</span><strong>{Math.round(testRatio * 100)}%</strong><small>{runs.length} expérience{runs.length > 1 ? 's' : ''} lancée{runs.length > 1 ? 's' : ''}</small></div>{runs.length === 0 ? <div className="empty-experiment">Lance une expérience pour entraîner puis tester un modèle.</div> : <div className="run-history">{runs.map((run, index) => <div key={`${run.seed}-${index}`}><span>#{index + 1}</span><i style={{ width: `${run.testScore * 100}%` }} /><strong>{Math.round(run.testScore * 100)}%</strong></div>)}</div>}<button className="secondary-button inline-button" onClick={runExperiment}>▶ Entraîner + tester</button></div>}>
        ...
        {latest && <div className="metric-grid"><div className="metric"><strong>{Math.round(latest.trainScore * 100)}%</strong><span>train · {latest.trainCount} tirs</span></div><div className="metric"><strong>{Math.round(latest.testScore * 100)}%</strong><span>test · {latest.testCount} tirs</span></div></div>}
        {runs.length >= 3 && <div className="feedback good"><strong>Tu as maintenant plusieurs mesures.</strong><span>Sur tes essais, le test varie de {Math.round(minScore * 100)}% à {Math.round(maxScore * 100)}%. Avec aussi peu de tirs, le groupe caché change beaucoup le résultat.</span></div>}
        <UnderTheHood><p>Le navigateur sépare les buts et les tirs ratés, mélange chaque groupe, réserve la proportion que tu as choisie pour le test, puis réentraîne le <strong>même modèle probabiliste</strong> uniquement sur le reste. La famille exacte du modèle viendra plus tard&nbsp;: ici, seule la logique train/test compte.</p></UnderTheHood>
  ...
  return (
    <LabShell visual={<div className="definition-visual stacked-definition"><span>apprendre sur</span><strong>TRAIN</strong><span className="flow-arrow">↓</span><span>réussir aussi sur</span><strong>INCONNU</strong></div>}>
